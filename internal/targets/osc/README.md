...
showbridge `ActionConfig` → one OSC message (bundles intentionally not used —
...
Registered in `cmd/showbridge/main.go` via blank import
(`_ "internal/targets/osc"`). Removing the import removes the module from the
binary — showbridge core runs fine without it.
