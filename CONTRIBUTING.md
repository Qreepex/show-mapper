...
Thanks for helping build showbridge! This file covers process; architectural
...
1. Verify the note/CC/LED map **physically**: `showbridge midi monitor <board>`
   while pressing/moving every control; check LED messages with a MIDI monitor
   (e.g. showbridge's `midi monitor` in send direction is not needed — LEDs are
   sent by the app).
